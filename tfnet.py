...
import math
from attention import Patten, Catten
...
class FeatureNorm(nn.Module):  # 归一化
...
class TFNet(nn.Module):
    def __init__(self, device, input_width, input_height, input_channels: int):
        super().__init__()
        if input_width % 8 != 0 or input_width % 8 != 0:
        ...
        self.seg_mask = nn.Sequential(
            ...
            FeatureNorm(num_features=1, eps=0.01, include_bias=False)
        )
        ...
        self.global_avg_pool_seg = nn.AvgPool2d(kernel_size=(self.input_height / 8, input_width / 8))
